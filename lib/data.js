...
class Data {
    ...
    NewWidthAndStep(newSize, newResolution) {
        console.log("Generating new data");
        this.size = newSize;
        this.resolution = newResolution;

        this.width = newResolution+1;
        this.step = newSize/newResolution;
    ...
    GeneratePoints() {
        ...
        for (let i=0;i<this.width;i++) {
            for(let j=0;j<this.width;j++) {
                const x = i*this.step;
                const z = j*this.step;
                const y = noise(x/(this.width*this.step),z/(this.width*this.step)) * 300;
