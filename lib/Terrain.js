...
class Terrain {
    ...
    constructor(size, resolution, maxHeight, scaleFactor) {
    ...
    GeneratePoints() {
        ...
        for (let i=0;i<width;i++) {
            for(let j=0;j<width;j++) {
                ...
                let noiseValue = this.Noise(x/(width*this.gridSize),z/(width*this.gridSize));

                for (let k=0;k<1;k++) {
                    noiseValue+= this.Noise((x+k*this.gridSize)/(width*this.gridSize),(z+k*this.gridSize)/(width*this.gridSize));
                } 

                const y = noiseValue*this.maxHeight;
